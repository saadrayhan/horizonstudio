  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-[72px] border-b border-border">
        <div className="mx-auto px-[50px] py-16 md:py-20 flex items-center justify-between">
          <motion.h1
      ...
      <section className="border-b border-border">
        <div className="mx-auto px-[50px] py-4">
        ...
        <div className="mx-auto border-t border-border">
          <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr]">
            <Reveal className="border-r border-border">
              <Link to={`/blog/${featured[0].slug}`} className="group px-[50px] py-10 block">
            ...
            <StaggerContainer className="flex flex-col">
              {recent.map((p) => (
                <StaggerItem key={p.slug}>
                  <Link
                    to={`/blog/${p.slug}`}
                    className="list-row group px-[50px] py-5 border-t border-border first:border-t-0 flex items-center justify-between"
                  >
      ...
      <section className="border-b border-border">
        <div className="mx-auto">
          <Reveal className="px-[50px] py-12 flex items-center justify-between">
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 border-t border-border">
            {allPosts.map((p, i) => (
              <StaggerItem key={p.slug}>
                <Link
                  to={`/blog/${p.slug}`}
                  className="group px-[50px] py-8 border-r border-border border-b last:border-r-0 [&:nth-child(3n)]:border-r-0 block hover:bg-secondary/40 transition-colors"
                >
